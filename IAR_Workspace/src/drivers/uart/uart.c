...
*  备    注：
*************************************************************************/
char uart_myGetChar (UARTn uratn)
{
//    u32 i = 0;

//    while(++i < 0xfffff)                                         //时间限制
//    {
...
*  备    注：
*************************************************************************/
void sendValue(UARTn uratn,u8 value)
{
	u8 cmdf[2] = {CMD_WARE, ~CMD_WARE};    //串口调试 使用的前命令
        u8 cmdr[2] = {~CMD_WARE, CMD_WARE};    //串口调试 使用的后命令
	
	uart_sendN  (uratn,cmdf,sizeof(cmdf));
	uart_putchar(uratn,(u8)(value));
	uart_sendN  (uratn,cmdr,sizeof(cmdr));
}
